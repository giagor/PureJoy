...
class DailyRecommendPlayListAdapter :
    BaseQuickAdapter<DailyRecommendPlayList, BaseDataBindingHolder<ItemHomeDailyRecommendPlaylistBinding>>(
        R.layout.item_home_daily_recommend_playlist
    ) {

    override fun convert(
        holder: BaseDataBindingHolder<ItemHomeDailyRecommendPlaylistBinding>,
        item: DailyRecommendPlayList
    ) {
        // 获取Binding
     ...
     * */
    fun setData(list: List<DailyRecommendPlayList>) {
        setList(list)
    }
}